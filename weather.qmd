...
library(fable)
library(glue)
knitr::opts_chunk$set(message=FALSE) # eval=FALSE to omit running
```
...
```{r}
first4 <- unique(null_temperature$site_id)[1:6]
...
Forecast uncertainty from ensemble distribution alone:
...
```{r}
fit <- lm(temperature ~ air_temperature, data = past2)
